document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM Elements ---
    const usageCounterDisplay = document.getElementById('usage-count');
    ...
    const ibanCalculator = {
        group: document.getElementById('ccc-input-group'),
        entidad: document.getElementById('ccc-entidad'),
        oficina: document.getElementById('ccc-oficina'),
        dc: document.getElementById('ccc-dc'),
        cuenta: document.getElementById('ccc-cuenta'),
        inputs: Array.from(document.querySelectorAll('#ccc-input-group input')),
        ibanResult: document.getElementById('iban-result'),
    };
    ...
    // --- Usage Counter Logic ---
    const USAGE_COUNT_KEY = 'calculatorUsageCount';
    let usageCount = 0;

    const updateUsageCountUI = () => {
        if (usageCounterDisplay) {
            usageCounterDisplay.textContent = usageCount;
        }
    };
    ...
    const loadUsageCount = () => {
        ...
        usageCount = savedCount ? parseInt(savedCount, 10) : 0;
        updateUsageCountUI();
    };
    
    const debouncedIncrement = debounce(incrementUsageCount);
    ...
    function switchCalculator(targetId) {
        ...
        if(resetButton) {
            const showReset = ['payroll-calculator', 'iban-calculator'].includes(targetId);
    ...
    // --- IBAN Calculator Logic ---
    function resetIbanCalculator() {
        if (!ibanCalculator.group) return;
        ibanCalculator.inputs.forEach(input => input.value = '');
        calculateIban();
        if (ibanCalculator.entidad) {
            ibanCalculator.entidad.focus();
        }
    }

    function calculateControlDigit(numberString) {
    ...
    function validateCCC(entidad, oficina, dc, cuenta) {
        if (entidad.length !== 4 || oficina.length !== 4 || dc.length !== 2 || cuenta.length !== 10) {
            return false;
        }

        const firstDigit = calculateControlDigit(`00${entidad}${oficina}`);
        const secondDigit = calculateControlDigit(cuenta);
        
        return dc === `${firstDigit}${secondDigit}`;
    }

    function calculateIban() {
        if (!ibanCalculator.group) return;

        const entidad = ibanCalculator.entidad.value;
        const oficina = ibanCalculator.oficina.value;
        const dc = ibanCalculator.dc.value;
        const cuenta = ibanCalculator.cuenta.value;
        const ccc = `${entidad}${oficina}${dc}${cuenta}`;
        
        ibanCalculator.group.classList.remove('valid', 'invalid');
        ibanCalculator.ibanResult.classList.remove('invalid');
        
        if (ccc.length === 0) {
            ibanCalculator.ibanResult.textContent = 'ESXX XXXX XXXX XXXX XXXX XXXX';
            return;
        }

        if (ccc.length < 20) {
            ibanCalculator.ibanResult.textContent = 'Completa los 20 dígitos del CCC';
            return;
        }

        const isValid = validateCCC(entidad, oficina, dc, cuenta);

        if (isValid) {
            ibanCalculator.group.classList.add('valid');
            const checkString = ccc + '142800'; // 14 for 'E', 28 for 'S'
            try {
                const checkNumber = BigInt(checkString);
                const remainder = checkNumber % 97n;
                const controlDigits = (98n - remainder).toString().padStart(2, '0');
                
                const iban = `ES${controlDigits}${ccc}`;
                const formattedIban = iban.match(/.{1,4}/g).join(' ');
                
                ibanCalculator.ibanResult.textContent = formattedIban;
            } catch (e) {
                ibanCalculator.ibanResult.textContent = 'Error en el cálculo';
            }
        } else {
            ibanCalculator.group.classList.add('invalid');
            ibanCalculator.ibanResult.classList.add('invalid');
            ibanCalculator.ibanResult.textContent = 'El CCC no es válido. Revisa los dígitos de control.';
        }
    }

    function initIbanCalculator() {
        if (!ibanCalculator.group) return;

        ibanCalculator.inputs.forEach((input, index) => {
            // Auto-jump to next input
            input.addEventListener('input', (e) => {
                const { value, maxLength } = e.target;
                if (value.length >= maxLength && index < ibanCalculator.inputs.length - 1) {
                    ibanCalculator.inputs[index + 1].focus();
                }
                calculateIban();
                debouncedIncrement();
            });
    ...
    function init() {
        ...
        const loanComparatorHandler = () => {
            calculateLoan();
            debouncedIncrement();
        };
        const loanComparatorNumericInputs = [
            loanCalculator.term1, loanCalculator.interest1, loanCalculator.openingFee1, 
            loanCalculator.term2, loanCalculator.interest2, loanCalculator.openingFee2
        ];
        loanComparatorNumericInputs.forEach(input => {
            if(input) input.addEventListener('input', loanComparatorHandler);
        });
        if (loanCalculator.amount1) {
            loanCalculator.amount1.addEventListener('input', (e) => {
                e.target.value = formatAmount(e.target.value);
                loanComparatorHandler();
            });
        }
        if (loanCalculator.amount2) {
            loanCalculator.amount2.addEventListener('input', (e) => {
                e.target.value = formatAmount(e.target.value);
                loanComparatorHandler();
            });
        }
        ...
        const singleLoanHandler = () => {
            calculateSingleLoan();
            debouncedIncrement();
        };
        const singleLoanNumericInputs = [singleLoanCalculator.interest, singleLoanCalculator.term];
        singleLoanNumericInputs.forEach(input => {
            if (input) input.addEventListener('input', singleLoanHandler);
        });
        if (singleLoanCalculator.amount) {
            singleLoanCalculator.amount.addEventListener('input', (e) => {
                e.target.value = formatAmount(e.target.value);
                singleLoanHandler();
            });
        }
        ...
        if (globalResetButton) {
            globalResetButton.addEventListener('click', () => {
                const activeCalculatorLink = document.querySelector('.tool-link.active');
                if (!activeCalculatorLink) return;

                const activeCalculatorId = activeCalculatorLink.dataset.calculator;

                if (activeCalculatorId === 'payroll-calculator') {
                    resetPayrollCalculator();
                } else if (activeCalculatorId === 'iban-calculator') {
                    resetIbanCalculator();
                }
            });
        }
